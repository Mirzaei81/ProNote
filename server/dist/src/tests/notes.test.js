import { appTesting } from "../Testsetup.js";
...
describe('Note Routes', ()=>{
    it('should not create a duplicate note title', async ()=>{
        const response = await appTesting.post('/note').set('Authorization', 'Bearer ' + token).send({
            title: 'My New Note',
    ...
    it('should update an existing note', async ()=>{
        const response = await appTesting.put('/note/2') // Replace with the actual note ID
    ...
    it('should delete a Testednote', async ()=>{
        await appTesting.post('/note').set('Authorization', 'Bearer ' + token).send({
            title: 'My Deleted Note',
            body: 'This is the content of my note.'
        });
...
describe('GET All Notes', ()=>{
    it('should retrieve all notes for a user', async ()=>{
        const response = await appTesting.get('/note').set('Authorization', 'Bearer ' + token);
        expect(response.status).toBe(200);
        expect(response.body.message).toBe('Succesfull');
