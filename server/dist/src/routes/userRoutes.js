...
UserRouter.post('/register', async (req, res)=>{
    ...
    try {
        ...
        const [count] = await req.conn.query('SELECT COUNT(*) FROM user_table WHERE username = ? AND email= ?', [
            username,
            email
        ]);
        if (count[0]["COUNT(*)"] > 0) {
...
UserRouter.post('/login', async (req, res)=>{
    ...
    try {
        ...
        const [data, fields] = await req.conn.query('select * from `user_table` where `username` = ? limit 1', [
            username
        ]);
        if (data.length === 0) {
            req.conn.release();
